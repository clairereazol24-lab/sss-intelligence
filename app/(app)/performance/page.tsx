    ...
    <table className="w-full text-sm table-fixed">
      <thead>
        <tr className="bg-gray-50 text-left">
          ...
          <th className="px-2 py-2 text-gray-500 font-medium w-[42%]">Store</th>
          <th className="px-2 py-2 text-gray-500 font-medium w-[22%]">DSP</th>
          <th className="px-2 py-2 text-gray-500 font-medium text-right w-[28%]">{metricLabel}</th>
        </tr>
      </thead>
      <tbody>
        {rows.map((s, i) => (
          <tr key={s.sub_affiliate} className="border-t border-gray-50 hover:bg-gray-50">
            <td className="px-2 py-2 text-gray-400 font-medium">{i + 1}</td>
            <td className="px-2 py-2">
              <div className="font-medium text-gray-800">{s.store_name}</div>
              <div className="text-xs text-gray-400">{s.sub_affiliate}</div>
            </td>
            <td className="px-2 py-2 text-gray-600 truncate">{s.dsp || '—'}</td>
            <td className="px-2 py-2 text-right font-medium text-gray-800">{metric(s)}</td>
          </tr>
        ))}
        {rows.length === 0 && (
  ...
  return (
    <table className="w-full text-sm table-fixed">
      <thead>
        <tr className="bg-gray-50 text-left">
          <th className="px-2 py-2 text-gray-500 font-medium w-[8%]">#</th>
          <th className="px-2 py-2 text-gray-500 font-medium w-[30%]">DSP</th>
          <th className="px-2 py-2 text-gray-500 font-medium w-[27%]">Partner</th>
          <th className="px-2 py-2 text-gray-500 font-medium text-right w-[35%]">{metricLabel}</th>
        </tr>
      </thead>
      <tbody>
        {rows.map((d, i) => (
          <tr key={`${d.dsp}-${d.partner}`} className="border-t border-gray-50 hover:bg-gray-50">
            <td className="px-2 py-2 text-gray-400 font-medium">{i + 1}</td>
            <td className="px-2 py-2 font-medium text-gray-800 truncate">{d.dsp}</td>
            <td className="px-2 py-2 truncate">
              {d.partner ? <span className="bg-blue-50 text-blue-700 text-xs px-2 py-0.5 rounded">{d.partner}</span> : '—'}
            </td>
            <td className="px-2 py-2 text-right font-medium text-gray-800">{metric(d)}</td>
          </tr>
        ))}
        {rows.length === 0 && (
          <tr><td colSpan={4} className="px-4 py-12 text-center text-gray-400">No data. Upload a CSV first.</td></tr>
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
      <div className="px-4 py-3 border-b border-gray-100">
        <h2 className="font-semibold text-gray-700 text-sm">{emoji} {title}</h2>
      </div>
