...
export default function SSSDataClient({ partner }: { partner: string }) {
  ...
  const fetchOverall = async (from: string, to: string) => {
    ...
    try {
      const res = await fetch(`/api/performance${buildQuery(from, to)}`)
  ...
  const handleExport = async () => {
    ...
    try {
      const params = new URLSearchParams()
      if (overallFrom && overallTo) { params.set('from', overallFrom); params.set('to', overallTo) }
      params.set('partner', partner)
      ...
      a.href = url
      a.download = `performance_data_${partner.replace(/\s+/g, '_').toLowerCase()}.csv`
  ...
  const handleCancel = () => {
    setFile(null); setParsed([]); setHeaders([]); setHasPartner(false); setHasDSP(false)
    setMode('new'); setConfirming(false); setError(null)
  ...
  const handleUploadClick = () => {
    ...
    if (!period || period.includes('undefined') || period === '-') { setError('Please select a valid period.'); return }
    if (mode === 'update' && !confirming) { setConfirming(true); return }
    performUpload()
  }
  ...
  const performUpload = async () => {
    ...
    if (data.error) {
      ...
      setParsed([]); setFile(null); setMode('new')
      if (fileRef.current) fileRef.current.value = ''
    ...
    <div className="p-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-xs text-gray-400 dark:text-gray-500">SSS Data</p>
        </div>
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl p-6 w-full max-w-3xl max-h-[90vh] overflow-y-auto shadow-xl dark:bg-gray-800">
            ...
            <div className="mb-4 space-y-2">
              {!hasPartner && <div className="bg-yellow-50 border border-yellow-200 text-yellow-800 text-sm px-4 py-2 rounded-lg dark:bg-yellow-900/30 dark:border-yellow-800 dark:text-yellow-400">⚠️ No <strong>Partner</strong> column detected.</div>}
              {!hasDSP && <div className="bg-yellow-50 border border-yellow-200 text-yellow-800 text-sm px-4 py-2 rounded-lg dark:bg-yellow-900/30 dark:border-yellow-800 dark:text-yellow-400">⚠️ No <strong>DSP</strong> column detected.</div>}
